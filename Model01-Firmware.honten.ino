...
const Key keymaps[][ROWS][COLS] PROGMEM = {
  ...
  (___,          Key_1, Key_2, Key_3, Key_4, Key_5, Key_LEDEffectNext,
   ...
   M(MACRO_BATTERFLY),  Key_N, Key_M, Key_Comma, Key_Period,    Key_Slash,     Key_Minus,
...
static void anyKeyMacro(uint8_t keyState) {
  ...
  if (keyToggledOn(keyState)) {
    Key key;
    key.flags = SHIFT_HELD | CTRL_HELD;
    key.keyCode = Key_J.keyCode;
    kaleidoscope::hid::pressKey(key);
  }
}

static void batterflyKeyMacro(uint8_t keyState) {
  if (keyToggledOn(keyState)) {
    Key key;
    key.flags = SHIFT_HELD | CTRL_HELD;
    key.keyCode = Key_Semicolon.keyCode;
    kaleidoscope::hid::pressKey(key);
  }
}
...
const macro_t *macroAction(uint8_t macroIndex, uint8_t keyState) {
  ...
  case MACRO_BATTERFLY:
    batterflyKeyMacro(keyState);
    break;
  }
  return MACRO_NONE;
}
